...
def tainNetwork():
    ...
    while "ticky" != "tacky":
        ...
        for i in range(episodes):
            completeGame, victory = playaGame(
                e, sess, inputState, prediction, Qoutputs)
            ...
            for i in GamesList:
                ...
                while loop_in < len_complete_game:
                    ...
                    if loop_in == 0:
                        ...
                        nextQ = sess.run(Qoutputs, feed_dict={
                                         inputState: [nextState]})
                        maxNextQ = np.max(nextQ)
                        game_reward = GAMMA * (maxNextQ)

                    targetQ = sess.run(Qoutputs, feed_dict={
                    ...
                    t_loss = sess.run([train_step, Qoutputs, loss], feed_dict={
                                      inputState: [currentState], targetQOutputs: targetQ})
                    total_loss += t_loss[2]
        ...
        if e > -0.2:
            ...
            e = random.choice([0.1, 0.05, 0.06, 0.07, 0.15,
                              0.03, 0.20, 0.25, 0.5, 0.4])
...
def playaGame(e, sess, inputState, prediction, Qoutputs):
    ...
    if(turn == -1):
        ...
        best_index, _ = sess.run([prediction, Qoutputs], feed_dict={
                                 inputState: [np.array(np.copy(myList).reshape(-1))]})
    ...
    while(True):
        ...
        pred, _ = sess.run([prediction, Qoutputs], feed_dict={
                           inputState: [temp_copy]})
        ...
        if(isFalsePrediction == True):
            ...
            action = random.choice(
                [selectedRandomIndex, pred, pred, pred, pred])
